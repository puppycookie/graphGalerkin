...
class Mesh(object):
    def __init__(self, etype, xcg, e2vcg, e2vcg0,e2bnd, ndim=None,
                 qrule='gl', nq0=None):
        ...
        self.e2vcg0=e2vcg0.copy('F')
        self.e2bnd=e2bnd.copy('F')
        self.ndim=ndim
        self.nsd=xcg.shape[0]
        print('self.nsd',self.nsd)
        if etype == 'hcube':
        ...
        if self.ndim==self.nsd:
            self.e2e=self.setup_mesh_elem2elem_conn(self.e2vcg0,self.lfcnsp.geom.f2n)
            print('self.e2vcg0',self.e2vcg0)
    ...
    def create_transf_data_ndim(self,geom,xcg,e2vcg,e2bnd,e2e):
        ...
        ndim=xcg.shape[0] # 维度
        nnode_per_elem,nelem=e2vcg.shape # 每个单元的节点数 9，单元数 4
        nx=geom.Qvv.shape[2] # 体积积分点数 36
        nxf=geom.Qvf.shape[2] # 面积积分点数 6
        nf=geom.geom.f2n.shape[1] # 面数 4

        # 计算积分点坐标
        xe=np.zeros((ndim, nnode_per_elem, nelem)) # 实际节点坐标，初始化


        xq=np.zeros((ndim, nx, nelem)) # 实际体积分点坐标，初始化
        xqf=np.zeros((ndim, nxf, nf, nelem)) # 面积积分点坐标，初始化
        detG=np.zeros((nx, nelem)) # 雅克比行列式，初始化
