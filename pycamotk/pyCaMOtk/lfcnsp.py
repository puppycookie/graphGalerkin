...
class LocalFunctionSpace(object):
    def __init__(self, polysp, nsd, porder, qrule0='gl', nq0=None, **kwargs):
        ...
        if nq0 is None: nq0=int((porder+1)/2)*3#int(np.max([3*porder,1]))
        self.nq0 = nq0
        # porder=2, nq0==6
        
        # Create quadrature weights/nodes for reference volume, face, edge
        # 创建【参考体积、面、边】的积分权重和节点
        ...
        if nsd > 1:
            ...
            rq = np.zeros((1,nq0)) # （1，6）
        
        if polysp=='P':
            self.wqv,zq=qrule_mltdim_simp(nsd,nq0,qrule0)
        elif polysp=='Q':
            self.wqv,zq=qrule_mltdim_hcube(nsd,nq0,qrule0) # 体积分点权重和坐标，36个点和权重一一对应
        else:
            raise ValueError('Polynomial space is not supported!')
        self.rq=rq
        self.zq=zq
        ...
        self.zqf = np.zeros((nsd, rq.shape[1], self.geom.f2n.shape[1])) # 面积分点坐标。形状是(空间维度, 积分点个数, 面的个数)
        if nsd > 1:
            if polysp=='Q':
        ...
        if polysp=='Q':
            ...
            self.Qvv = self.eval_basis_functions(self.zqv) # 体的基函数 维度是
