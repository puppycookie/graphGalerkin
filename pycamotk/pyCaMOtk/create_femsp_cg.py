...
class create_femsp_cg(object):
	...
	def __init__(self,prob,msh0,ptest,e2vcg_test,
		...
		self.prob=prob 
		self.msh0=msh0 #这里的msh0是一个Mesh对象
		self.ptest=ptest #
		self.e2vcg_test=e2vcg_trial #这里由于是连续Galerkin方法，所以测试函数和试探函数是一样的
		self.ptrial=ptrial 
		...
		self.lfcnsp_eqn=LocalFunctionSpace(self.polysp, #
			                               self.ndim, 
			                               self.ptest, #测试函数的多项式次数
			                               'gl',
			                               self.nquad_per_dim) #创建局部函数空间
		self.lfcnsp_var=LocalFunctionSpace(self.polysp,
										   self.ndim,
										   self.ptrial,  #试探函数的多项式次数
										   'gl',
										   self.nquad_per_dim)
		self.lfcnsp_msh=self.msh0.lfcnsp
		self.ldof2gdof_eqn=create_ldof2gdof_cg(self.neqn,self.e2vcg_test) # 创建局部自由度到全局自由度的映射，对于方程
		self.ldof2gdof_var=create_ldof2gdof_cg(self.nvar,self.e2vcg_trial) # 创建局部自由度到全局自由度的映射，对于变量
		self.ldof2gdof_msh=create_ldof2gdof_cg(self.ndim,self.msh0.e2vcg) # 创建局部自由度到全局自由度的映射，对于网格
