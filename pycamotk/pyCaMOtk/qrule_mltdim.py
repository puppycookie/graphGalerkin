...
def qrule_mltdim_hcube_from_onedim(ndim, w0, z0):
    ...
    quadrature rule 
    # 一维积分点的权重和节点 -> 通过张量积 -> 得到多维积分点的权重和节点
    
    Input arguments
    ---------------
    ndim : int
      Number of spatial dimensions 空间维度
    w0 : ndarray, size = (N0,)
    ...
    """
    w = tensprod_scalar_unif(w0, ndim, flatten=True) # 一维积分点的权重 -> 多维积分点的权重
    z = tensprod_vector_unif(z0, ndim, flatten=True) # 一维积分点的节点 -> 多维积分点的节点
...
def qrule_mltdim_simp_from_hcube(w_hcube0, z_hcube0):
    ...
    对于超立方体[-1, 1]^ndim的积分点权重和节点，转换为单位超立方体[0, 1]^ndim的积分点权重和节点
    Input arguments
    ---------------
...
def qrule_mltdim_hcube(ndim, nq0, qrule0):
    ...
    Return values
    -------------
    w : ndarray, size = N = nq0**ndim (6**2 = 36) 
      Quadrature weights
    z : ndarray, size = (ndim, N) = (2, 36), 2维空间的36个积分点
      Quadrature points
    ...
    """
    w0, z0 = qrule_onedim(nq0, qrule0) # 一维积分点的权重和节点，维度是6
