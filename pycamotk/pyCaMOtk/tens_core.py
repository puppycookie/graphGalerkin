...
def tensprod_vector_unif(v0, N, flatten=True, rstr2simp=False):
    ...
    n = len(v0)
    dims = [n for k in range(N)]
    idx = [mltidx_from_linidx(dims, k) for k in range(n**N)] # 生成多维索引
    idx = [idx0 for idx0 in idx if sum(idx0)<n] # 限制在单位超立方体内
    idx = [linidx_from_mltidx(dims, idx0) for idx0 in idx] # 生成线性索引
    return v[:, idx] 
