...
class create_elem_strct(object):
	...
	def __init__(self,eqn,lfcnsp_eqn,lfcnsp_var):
		self.eqn=eqn
		self.lfcnsp_eqn=lfcnsp_eqn # 方程的局部函数空间
		self.lfcnsp_var=lfcnsp_var # 变量的局部函数空间
		...
		self.Tv_var_ref=self.var_elem.Tv # 试探函数的基函数
		self.Tvf_var_ref=self.var_elem.Tvf # 试探函数的面的基函数
