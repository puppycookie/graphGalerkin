...
for Re in ReList:
	...
	for i in range(len(dbc_idx2)):
		...
		#boundaryinletIDX.append(2*dbc_idx2[i])
		dbc_val.append(dbc_val2[i])
	for i in range(len(dbc_idx2)):
		dbc_idx.append(2*dbc_idx2[i]+1)
		boundaryinletIDX.append(2*dbc_idx2[i]+1)
		dbc_val.append(fvol(xcg[0,dbc_idx2[i]])) #inletVelocity
	...
	if Re==ReList[-1]:
		...
		#####
		obsidx=np.loadtxt('./obsidx1.txt').astype('int')#np.asarray(sample([i for i in range(ndofUP) if i not in dbc_idx], 19)) #np.loadtxt('./obsidx.txt').astype('int')#
		#####
		
		
		#np.savetxt('obsidx.txt',obsidx)
		dbc_idx_new=np.hstack([obsidx,dbc_idx])
		dbc_idx_new=[i for i in dbc_idx_new if i not in boundaryinletIDX]
		dbc_val_new=U[dbc_idx_new]
		#xcg[:,[i//2+i%2 for i in dbc_idx_new]]


		fig=plt.figure()
		ax=plt.subplot(1,1,1)
		visualize_fem(ax,msh,uabs[e2vcg],{"plot_elem":False,"nref":4},[])
		vidx=[i//2 for i in obsidx if i//2<441]
		ax.plot(xcg[0,vidx],
		        xcg[1,vidx],'o')
...
## Recreate DBC to do the infer




dbc_idx_soft=[i for i in dbc_idx if i not in boundaryinletIDX]
dbc_val_soft=U[dbc_idx_soft]

dbc_hard=create_dbc_strct(ndofUP,dbc_idx_new,dbc_val_new)
dbc_soft=create_dbc_strct(ndofUP,dbc_idx_soft,dbc_val_soft)





connectivity_uv=e2vcg2connectivity(e2vcg,'iso')
connectivity_p=e2vcg2connectivity(e2vcg2,'iso')
...
fcn=lambda u_:TensorFEMCore.create_fem_resjac('cg',
							...
							msh.e2e,femsp_gcnn.spmat,dbc_hard,[i for i in range(ndofUP) if i not in dbc_idx]) # 
LossF.append(fcn)
ii=0
Graph=[]
...
model=model.double()
[model,info]=solve_fem_GCNN(TrainDataloader,LossF,model,tol,maxit,boundaryinletIDX)
