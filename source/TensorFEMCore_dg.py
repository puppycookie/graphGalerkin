...
def eval_unassembled_resjac_claw_dg(U, transf_data, elem, elem_data, ldof2gdof_var, e2e):
    ...
    # 强制转换为 PyTorch 张量并指定设备
    device = U.device if torch.is_tensor(U) else 'cpu'

    # 初始化 U 为 PyTorch 张量
    if U is None or len(U) == 0 or U == []:
        ndof_var = torch.max(ldof2gdof_var).item() + 1
        U = torch.zeros((ndof_var, 1), device=device, dtype=torch.float64)

    # 初始化 Re 和 dRe
    Re = torch.zeros((neqn_per_elem, nelem), device=device, dtype=torch.float64)
    Re_intface = torch.zeros((neqn_per_elem, nelem), device=device, dtype=torch.float64)
    Re_vol = torch.zeros((neqn_per_elem, nelem), device=device, dtype=torch.float64)
    dRe = torch.zeros((neqn_per_elem, nvar_per_elem, nf+1, nelem), device=device, dtype=torch.float64)

    for e in range(nelem):
        Ue = U[ldof2gdof_var[:, e]]
        Uef = torch.zeros((nvar_per_elem, nf+1), device=device, dtype=torch.float64)
        Uef[:, 0] = ReshapeFix(Ue, Uef[:, 0].shape, order='F')

        for f in range(nf):
            if np.isnan(e2e[f, 0, e]):  # 使用 torch.isna
                Uef[:, 1+f] = torch.nan  # PyTorch 的 NaN
                continue
            ep = int(e2e[f, 0, e])
            Uef[:, 1+f] = ReshapeFix(U[ldof2gdof_var[:, ep]],Uef[:, 1+f].shape, order='F')
        ...
        dRe2_ = torch.tensor(dRe2_,device=Re0_.device)

        Re[:, e] = ReshapeFix(Re0_ + Re1_ + Re2_, Re[:, e].shape, order='F')
        Re_intface[:, e] = ReshapeFix(Re2_, Re_intface[:, e].shape, order='F')
        Re_vol[:, e] = ReshapeFix(Re0_, Re_vol[:, e].shape, order='F')
        dRe[:, :, 0, e] = ReshapeFix(dRe0_ + dRe1_ + dRe2_[:, :, 0], dRe[:, :, 0, e].shape, order='F')
		
        #Re[:, e] = ReshapeFix(Re0_ + Re1_ , Re[:, e].shape, order='F')
        #dRe[:, :, 0, e] = ReshapeFix(dRe0_ + dRe1_ , dRe[:, :, 0, e].shape, order='F')
        for f in range(nf):
            if np.isnan(e2e[f,0,e]):
                continue
            dRe[:,:,1+f,e]=+dRe[:,:,1+f,e]+dRe2_[:,:,1+f]
    return Re,dRe,Re_intface,Re_vol
...
def create_fem_resjac(fespc,Uf,transf_data,elem,elem_data,
	...
	if fespc=='cg' or fespc=='CG':
		Re,dRe,Re_intface,Re_vol=eval_unassembled_resjac_claw_dg(U,transf_data,elem,elem_data,
			                                   ldof2gdof_var,e2e)
	...
	R_int = assemble_nobc_vec(Re_intface,ldof2gdof_eqn)
	R_vol = assemble_nobc_vec(Re_vol,ldof2gdof_eqn)
	if enforce_idx==None:
		Rf=R[free_idx]
	else:
		Rf=R[enforce_idx]
...
def intg_elem_claw_intface(Uef,transf_data,elem,elem_data,e2e,e,elst):
	e2e=e2e[:,:,e] #取出当前单元的面的连接信息
	# 第一维是面的编号
	# 第二维是面的连接信息
	# 第三维是单元的编号
	[neqn_per_elem,neqn,ndimP1,nqf,nf]=elem.Tvf_eqn_ref.shape
	[nvar_per_elem,nvar,_,_,_]=elem.Tvf_var_ref.shape
	ndim=ndimP1-1; wqf=elem.wqf; 
	perm=transf_data.perm[:,:,e] 
	sigf=transf_data.sigf[:,:,e]
	Re=np.zeros([neqn_per_elem,1])
	...
	for f in range(nf):
		if np.isnan(e2e[f,0]): #没有另一个单元，证明在整个区域的边界上
			continue
		fp=int(e2e[f,1]) #另一个面的索引
		TvfI=np.reshape(elem_data.Tvf_var_phys[:,:,:,:,f,elst[0]], #这个面本单元的基函数
						[nvar_per_elem,nvar*(ndim+1)*nqf],order='F')
		#转化成PyTorch张量
		#输出TvfI的形状
		TvfI_t=torch.from_numpy(TvfI).double().to('cuda')
		Uef_0=ReshapeFix(Uef[:,0],[9,1],order='F')
		UQqfI=ReshapeFix(torch.mm(TvfI_t.T,Uef_0),[nvar,ndim+1,nqf],order='F')
		TvfO=np.reshape(elem_data.Tvf_var_phys[:,:,:,:,fp,elst[(1+f)]], #另一个面的基函数
						[nvar_per_elem,nvar*(ndim+1)*nqf],order='F')
		TvfO_t=torch.from_numpy(TvfO).double().to('cuda')
		Uef_f=ReshapeFix(Uef[:,1+f],[9,1],order='F')
		UQqfO=ReshapeFix(torch.mm(TvfO_t.T,Uef_f),[nvar,ndim+1,nqf],order='F')
		UQqfO=UQqfO[:,:,tuple(perm[:,f].astype('int'))]
		for k in range(nqf): # 遍历这个面每个积分点
			x=transf_data.xqf[:,k,f,e]
			n=transf_data.n[:,k,f,e]
			parsI=elem_data.face_pars[:,k,f,elst[0]] 
			parsO=elem_data.face_pars[:,int(perm[k,f]),fp,elst[1+f]] 
			Fs, dFsdUQI, dFsdUQO = elem.eqn.numflux(
                UQqfI[:,:,k], parsI, UQqfO[:,:,k], parsO, x, n
            )
			# Fs是通过当前面和相邻面的基函数值计算得到的数值通量
			dFsdUQI=ReshapeFix(dFsdUQI,[neqn,nvar*(ndim+1)],order='F')
			dFsdUQO=ReshapeFix(dFsdUQO,[neqn,nvar*(ndim+1)],order='F')
			...
			TvfO=np.reshape(elem_data.Tvf_var_phys[:,:,:,int(perm[k,f]),fp,elst[1+f]],
							[nvar_per_elem,nvar*(ndim+1)],order='F')
			Fs=ReshapeFix(Fs,[neqn,1],order='F')
			#wf转化成PyTorch张量
			wf_k_f_TvfI0 = torch.from_numpy(wf[k, f] * TvfI0).to(Uef.device)
			Re = Re + ReshapeFix(torch.mm(wf_k_f_TvfI0, Fs), Re.shape, order='F')
...
def solve_SGD(DataLoader,LossF,model,tol,maxit,qoiidx,softidx,penaltyConstant,plotFlag='True'):
	...
	for epoch in range(maxit):
		print('epoch = ',epoch)
		startime=time.time()
		er,loss,loss_int,loss_vol,model=trainmodel(DataLoader,LossF,model,
		...
		Er.append(er);Loss.append(loss);Loss_int.append(loss_int);Loss_vol.append(loss_vol)
	...
	if plotFlag:
		...
		ax.plot(Loss_int,label='|Residual_int|')
		#ax.plot(Loss_vol,label='|Residual_vol|')
		ax.legend()
		ax.set_xlabel('Epoch')
		ax.set_yscale('log')
...
def trainmodel(DataLoader,LossF,model,optimizer,criterion,qoiidx,softidx,penaltyConstant):
	...
	for data in DataLoader:
		...
		tic=time.time()
		output = model(input)
		Re,dRe,dbc,Re_int,Re_vol=fcn(output)
		print('reint=',torch.norm(torch.abs(Re_int)))
		print('wallclock time of evl Res= ',time.time()-tic)
		intlist.append(torch.abs(Re_int))
		volist.append(torch.abs(Re_vol))
	...
	loss=ReList[0]*0
	loss_int=intlist[0]*0
	loss_vol=volist[0]*0
	for i in range(len(ReList)):
		loss=loss+ReList[i]
		loss_int=loss_int+intlist[i]
		loss_vol=loss_vol+volist[i]
	print('max Res=',loss.abs().max().item())
	#print(loss)
	loss=torch.norm(loss)#loss.max()#
	loss_int=torch.norm(loss_int)
	loss_vol=torch.norm(loss_vol)
	...
	return er_0/len(DataLoader),loss.norm().item()/len(DataLoader),loss_int.norm().item()/len(DataLoader),loss_vol.norm().item()/len(DataLoader),model
