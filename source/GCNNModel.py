		...
		for e in range(NnE):
			...
			for i in range(len(nodes)):
				for j in range(i + 1, len(nodes)):
					connectivity.append(np.asarray([nodes[i], nodes[j]]))
					connectivity.append(np.asarray([nodes[j], nodes[i]]))
		return torch.tensor(torch.from_numpy(np.asarray(connectivity).T).to('cuda'), dtype=torch.long)
...
class PossionNet(torch.nn.Module):
	def __init__(self,nci=3 ,nco=1,kk=10):
		super(PossionNet, self).__init__()
		self.conv1 = ChebConv(nci, 32,K=kk)
		self.conv2 = ChebConv(32, 64,K=kk)
		self.conv3 = ChebConv(64, 128,K=kk)
...
class LambdaLayer(nn.Module):
    ...
    def forward(self, x, edge_index):
        return self.lambd(x, edge_index)  # 忽略edge_index，只返回x
# 残差块定义（包含两个ChebConv层）
class ChebResBlock(nn.Module):
    def __init__(self, in_channels, mid_channels_1,mid_channels_2,mid_channels_3,mid_channels_4,
	 out_channels, kk, final_block=False):
        super().__init__()
        # 主路径8个卷积层
        self.conv1 = ChebConv(in_channels, mid_channels_1,K=kk)
        self.conv2 = ChebConv(mid_channels_1, mid_channels_2,K=kk)
        self.conv3 = ChebConv(mid_channels_2, mid_channels_3,K=kk)
        self.conv4 = ChebConv(mid_channels_3, mid_channels_4,K=kk)
        self.conv5 = ChebConv(mid_channels_4, mid_channels_3,K=kk)
        self.conv6 = ChebConv(mid_channels_3, mid_channels_2,K=kk)
        self.conv7 = ChebConv(mid_channels_2, mid_channels_1,K=kk)
        self.conv8 = ChebConv(mid_channels_1, out_channels,K=kk)

        # 跳跃连接处理（通道数变化时用ChebConv调整）
        self.shortcut = (
    ChebConv(in_channels, out_channels, K=kk) 
    if in_channels != out_channels 
    else LambdaLayer(lambda x, edge_index: x)  # 自定义恒等层
)
        
        self.final_block = final_block  # 是否最后一个块

    def forward(self, x, edge_index):
        identity = self.shortcut(x, edge_index)  # 跳跃连接
        ...
        out = F.relu(out)
        out = self.conv7(out, edge_index)
        out = F.relu(out)
        out = self.conv8(out, edge_index)

        out += identity  # 残差连接
        if not self.final_block:  # 非最后一个块加ReLU
            out = F.relu(out)
        return out

# 修改后的PossionNet（残差结构）
class PossionNetTime(torch.nn.Module):
    def __init__(self, nci=1, nco=1, kk=10):
        super(PossionNetTime, self).__init__()
        # 定义1个残差块（每个块包含两个卷积层）
        self.block1 = ChebResBlock(nci, 32,64,128, 256, nco, kk, final_block=True)
        
        # 可训练参数（原代码中的source）
        self.source = torch.nn.Parameter(torch.tensor([0.25]))
        self.source.requires_grad = True
    ...
    def _init_weights(self):
        try:  # 尝试使用自定义初始化
            self._apply_last_chance_init()
        except:  # 失败则用正交初始化
            self._apply_orthogonal_init()

    def _apply_last_chance_init(self):
        # 假设last_chance0/1是自定义初始化函数
        # 需要根据实际定义补充
        for name, module in self.named_modules():
            if isinstance(module, ChebConv):
                if 'block1' in name:  # 最后一个块
                    last_chance1(module)
                else:
                    last_chance0(module)
    ...
    def _apply_orthogonal_init(self):
        for name, module in self.named_modules():
            if isinstance(module, ChebConv):
                gain = (1.0 if 'block4' in name else 
